...
/**
 電圧管理(保持)用クラス
 1:デジタルI/OまたはPowerピンにパーツが接続されているかを調べる
 2:接続されているピンの行または列を指定された電圧値に変更
 3:2の行または列内にパーツが接続されているかを調べる
 4:接続されていた場合、2に戻る
 5:最終的にGNDピンに戻っていた場合"VOLT RUN"を実行させる
...
//    var partsDraw = PartsDraw()
    var flagPowerIn = 0 // PowerピンまたはデジタルI/Oにパーツが接続されているかどうかを調べるときに使用
    var flagPartIn = 0 // パーツ同士のつながりを調べるときに使用
    var flagGndIn = 0 // GNDピンに接続があるかどうかを調べる

    var searchInput = 0 // power_in用
    ...
    // - repeating: 中身
    // - count: 個数
    var voltControlArray = Array(repeating: 0, count: 90) // 回路内部の電圧管理配列
    var voltConnectedArray: [[Int]] = [[404], [405], [408], [409], [410], [411], [419], [420], [421], [422], [423], [424]]
    var voltInPinArray: [Int] = [412, 413, 414] // 入力ピンナンバー保持用

    /**
     voltPowerIn
     powerピンまたはデジタルI/Oにパーツの接続があるかどうかを調べ、接続があった場合にはflagPowerInから1(Int)を返り値
     - Parameters:
     - repeatNumber: 独自解釈のピン番号
     - Returns: Int型で0or1を返す
     */
    func voltPowerIn(repeatNumber: Int) -> Int {
        if flagPowerIn == 0 {
            ...
            /* ピン番号402、403がpowerピンを示しており、419~432がデジタルI/Oを示している */
    ...
    /**
     voltConnection
     パーツによって接続されているピンを調べる
     inPinNumber:pinNumberを引数として取ってきて使う形にしたい(願望)
     - Parameters:
     - inPinNumber: 独自解釈のピン番号
     - repeatNumber: リピート
     - Returns: 接続されているピンの番号
     */
